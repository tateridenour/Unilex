...
class GetRidOf(commands.Cog):
    ...
    async def getridof(self, ctx, *, message: str):
        ...
        # If nobody owns it
        passive_aggressive_denial_message = f'Nobody owns **{article_title}**, not even you.'
        ...
        if article_owner != ctx.author.id:
            print(ctx.author.id)
            print(article_owner)
            await ctx.send(f'{self.bot.get_user(article_owner)} owns **{article_title}**, not you.')
            return
        ...
        def checkyes(msg):
            ...
            return (
                ...
                bool(re.match(r"^y+e+s+$", msg.content)) or msg.content == 'y'
                or bool(re.match(r'^n+o+$', msg.content)) or msg.content == 'n'
        ...
        try:
            ...
            if re.match(r"^ye+s+$", content) or content == 'y':  # Confirm
                ...
                await ctx.send(f'Never mind, **{article_title}**. **{ctx.author}** missed you.')
            # Place your action here
        except asyncio.TimeoutError:
            await ctx.send("Cancelled delete request. No confirmation received in time.")
